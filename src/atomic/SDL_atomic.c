...
#if defined(_MSC_VER) && (_MSC_VER >= 1500) && (!SDL_ATOMIC_DISABLED)
#include <intrin.h>
#define HAVE_MSC_ATOMICS 1
#endif

#if defined(__MACOSX__) && (!SDL_ATOMIC_DISABLED)
#include <libkern/OSAtomic.h>
#endif

#if !defined(HAVE_GCC_ATOMICS) && defined(__SOLARIS__) && (!SDL_ATOMIC_DISABLED)
#include <atomic.h>
#endif
...
#if !defined(__ANDROID__) && (!SDL_ATOMIC_DISABLED)
#define HAVE_ATOMIC_LOAD_N 1
#endif
#endif
#elif defined(__GNUC__) && (!SDL_ATOMIC_DISABLED)
#if (__GNUC__ >= 5)
#define HAVE_ATOMIC_LOAD_N 1
#endif
#endif
...
SDL_bool SDL_AtomicCAS(SDL_atomic_t *a, int oldval, int newval)
{
    ...
    /* Single-threaded OS3 build — no mutex needed, no race possible */
    if (a->value == oldval) {
        a->value = newval;
        return SDL_TRUE;
    }
